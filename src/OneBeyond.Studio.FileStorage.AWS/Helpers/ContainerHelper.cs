...
internal static partial class ContainerHelper
{
        ...
        S3BaseStorageOptions options)
    {
        ...
        var blobServiceClient = string.IsNullOrWhiteSpace(options.AccountName)
            ...
            // TODO: 
            : new BlobServiceClient(new Uri($"https://{options.AccountName}.blob.core.windows.net"), new DefaultAWSCredential());
        ...
        TimeSpan sharedAccessDuration)
    {
        ...
        {
            throw new S3StorageException("BlobClient must be authorized with Shared Key credentials to create a service SAS.");
        }
    ...
    private static void ValidateOptions(S3BaseStorageOptions options)
    {
        if (string.IsNullOrWhiteSpace(options.ConnectionString) && string.IsNullOrWhiteSpace(options.AccountName))
        {
            throw new ArgumentException("At least one connection must be provided, " +
                "either the connection string or the account name (for AWS Identity usage).");
        }
    ...
    private static void ValidateContainerName(string? containerName)
    {
        ...
        {
            throw new S3StorageException("Container name cannot be empty.");
        }

        if (containerName.Length < 3 || containerName.Length > 63)
        {
            throw new S3StorageException("Container name must be between 3 and 63 characters in length.");
        }

        if (!ContainerCharacterRegex().IsMatch(containerName))
        {
            throw new S3StorageException("Container name can only contain lowercase letters, numbers or hyphens.");
        }

        if (!ContainerFullValidityRegex().IsMatch(containerName))
        {
            throw new S3StorageException("Container name must start and end with a number or letter and cannot contain multiple hyphens in sequence.");
        }
    }
